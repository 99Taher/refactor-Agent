...
def get_changed_files(repo_path: Path, base_ref: str) -> List[str]:
    # (fonction inchangée - je la laisse telle quelle)
...
def build_prompt(is_retry: bool = False) -> str:
    """Ton prompt d'origine, juste un peu renforcé sur la déduplication comme tu le voulais"""
    retry_prefix = (
        ...
        "Only modify Log.x / Logr.x / AppSTLogger lines and merge duplicates.\n\n"
    ) if is_retry else ""
    ...
    return (
        ...
        " - AppSTLogger.appendLogST(...) -> AppLogger.d/i/w/e(tag, msg) selon le niveau\n\n"
        "2. DEDUPLICATION RULE (CRITICAL - FAIS-LE PARTOUT):\n"
        " - Merge **tous** les appels AppLogger consécutifs ou très proches qui ont le **même tag ET le même message** en UN SEUL.\n"
        " - Exemple : plusieurs AppLogger.e(MODULE, 'text') d’affilée → ne garder que le premier.\n"
        " - Sois agressif même s’il y a des lignes vides entre eux.\n\n"
        "3. IMPORTS:\n"
...
def call_groq(code: str, chunk_index: Optional[int] = None, is_retry: bool = False) -> str:
    # fonction inchangée (sauf le label pour le log)
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if resp.status_code == 429:
                time.sleep(int(resp.headers.get("retry-after", RATE_LIMIT_BASE_WAIT * (2 ** attempt))))
                continue
    ...
    raise Exception(f"Max retries on {label}")
...
def validate_refactoring(original: str, refactored: str, filepath: str) -> tuple[bool, str]:
    ...
    orig_non_log = [l.strip() for l in original.splitlines() if l.strip() and not is_log_related(l)]
    new_non_log = [l.strip() for l in refactored.splitlines() if l.strip() and not is_log_related(l)]
    if orig_non_log == new_non_log or abs(len(orig_non_log) - len(new_non_log)) <= 2:
        return True, "ok"
...
def refactor_file(repo_path: Path, filepath: str) -> str:
    ...
    try:
        ...
        # === PLUS AUCUNE FONCTION PYTHON DE MERGE ===
        # Tout le merge des duplicons est fait par le LLM maintenant
        ...
        logger.info(f"✅ {filepath} refactorisé (tout par LLM)")
...
@app.get("/")
def root():
    return {"message": "Refactor Agent API Active", "version": "16.0-llm-only"}


@app.get("/health")
def health():
    return {"status": "healthy", "deduplication": "100% LLM", "groq_model": GROQ_MODEL}
