...
def build_prompt(is_retry: bool = False) -> str:
    ...
    return (
        ...
        "═══════════════════════════════════════════════════════════\n"
        "STEP 2 — DEDUPLICATE (read carefully, this is critical)\n"
        "═══════════════════════════════════════════════════════════\n"
        "After conversion, scan for consecutive AppLogger calls.\n"
        "Two calls are duplicates if BOTH conditions are true:\n"
        "  1. Same level (d, i, w, or e)\n"
        "  2. Identical arguments (tag AND message)\n\n"

        "RULE A — Same level + Same args → KEEP ONLY ONE:\n"
        "  INPUT:\n"
        "    AppLogger.i(TAG, \"connected\")\n"
        "    AppLogger.i(TAG, \"connected\")     ← duplicate\n"
        "    AppLogger.i(TAG, \"connected\")     ← duplicate\n"
        "  OUTPUT:\n"
        "    AppLogger.i(TAG, \"connected\")     ← keep 1\n\n"

        "RULE B — Different level + Same args → KEEP ALL:\n"
        "  INPUT:\n"
        "    AppLogger.d(TAG, \"status: $v\")\n"
        "    AppLogger.i(TAG, \"status: $v\")    ← different level\n"
        "    AppLogger.e(TAG, \"status: $v\")    ← different level\n"
        "  OUTPUT:\n"
        "    AppLogger.d(TAG, \"status: $v\")    ← keep (level d)\n"
        "    AppLogger.i(TAG, \"status: $v\")    ← keep (level i)\n"
        "    AppLogger.e(TAG, \"status: $v\")    ← keep (level e)\n\n"

        "RULE C — Same level + Different args → KEEP ALL:\n"
        "  INPUT:\n"
        "    AppLogger.e(TAG, \"connect failed\")\n"
        "    AppLogger.e(TAG, \"timeout\")       ← different message\n"
        "    AppLogger.e(TAG, \"retry\")         ← different message\n"
        "  OUTPUT:\n"
        "    AppLogger.e(TAG, \"connect failed\") ← keep (message A)\n"
        "    AppLogger.e(TAG, \"timeout\")        ← keep (message B)\n"
        "    AppLogger.e(TAG, \"retry\")          ← keep (message C)\n\n"

        "RULE D — Typos are DIFFERENT messages:\n"
        "  INPUT:\n"
        "    AppLogger.i(TAG, \"deviceEnabled $v\")\n"
        "    AppLogger.i(TAG, \"deviceEnabl $v\")  ← typo = different\n"
        "  OUTPUT:\n"
        "    AppLogger.i(TAG, \"deviceEnabled $v\") ← keep\n"
        "    AppLogger.i(TAG, \"deviceEnabl $v\")  ← keep (different)\n\n"
        ...
        "    AppLogger.e(MODULE, \"deviceHasLocationSettinffdfdffffdfgEnabled $v\")  ← Log.e converted\n"
        "  \n"
        "  STEP 2 (deduplicate):\n"
        "    AppLogger.i(MODULE, \"deviceHasLocationSettingEnabled $v\")  ← keep 1 of 5 (same level i, same args)\n"
        "    AppLogger.d(MODULE, \"deviceHasLocationSettingEnabled $v\")  ← keep (different level d)\n"
        "    AppLogger.e(MODULE, \"deviceHasLocationSettinffdfdffffdfgEnabled $v\")  ← keep (different message with typo)\n"
        "  \n"
