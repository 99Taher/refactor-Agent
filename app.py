...
def build_prompt(is_retry: bool = False) -> str:
    ...
    return (
        ...
        "Your mission: Migrate and deduplicate logging in this Android Kotlin code.\n\n"
        ...
        " • ONLY touch log-related lines and imports.\n\n"

        "═══════════════════════════════════════════════════════════\n"
        "STEP 1 — CONVERT all log calls to AppLogger\n"
        "═══════════════════════════════════════════════════════════\n"
        ...
        "  Logr.d/i/w/e(...)  → AppLogger.d/i/w/e(...)\n"
        ...
        "═══════════════════════════════════════════════════════════\n"
        "STEP 2 — DEDUPLICATE (READ THIS SECTION CAREFULLY)\n"
        "═══════════════════════════════════════════════════════════\n"
        
        "⚠️ CRITICAL RULE — Two AppLogger calls are duplicates ONLY IF:\n"
        "   1. They have the EXACT SAME LEVEL (.d, .i, .w, or .e)\n"
        "   2. AND they have IDENTICAL arguments (tag and message)\n\n"

        "⚠️ DO NOT MERGE if the level is different, even if the message is the same!\n"
        "⚠️ .d and .e are DIFFERENT levels → DO NOT MERGE\n"
        "⚠️ .i and .w are DIFFERENT levels → DO NOT MERGE\n"
        "⚠️ .d and .i are DIFFERENT levels → DO NOT MERGE\n\n"

        "RULE A — SAME level + SAME args → Merge to ONE:\n"
        "  INPUT:\n"
        "    AppLogger.d(TAG, \"connected\")  ← level d, message \"connected\"\n"
        "    AppLogger.d(TAG, \"connected\")  ← level d, message \"connected\" (DUPLICATE)\n"
        "    AppLogger.d(TAG, \"connected\")  ← level d, message \"connected\" (DUPLICATE)\n"
        "  OUTPUT:\n"
        "    AppLogger.d(TAG, \"connected\")  ← keep only 1\n\n"

        "RULE B — DIFFERENT level + SAME args → KEEP ALL (DO NOT MERGE!):\n"
        "  INPUT:\n"
        "    AppLogger.d(TAG, \"status\")     ← level d\n"
        "    AppLogger.i(TAG, \"status\")     ← level i (DIFFERENT from d)\n"
        "    AppLogger.e(TAG, \"status\")     ← level e (DIFFERENT from d and i)\n"
        "  OUTPUT:\n"
        "    AppLogger.d(TAG, \"status\")     ← KEEP (level d)\n"
        "    AppLogger.i(TAG, \"status\")     ← KEEP (level i, DIFFERENT level!)\n"
        "    AppLogger.e(TAG, \"status\")     ← KEEP (level e, DIFFERENT level!)\n"
        "  ⚠️ These are NOT duplicates because levels differ!\n\n"

        "RULE C — SAME level + DIFFERENT args → KEEP ALL:\n"
        "  INPUT:\n"
        "    AppLogger.e(TAG, \"connect failed\")\n"
        "    AppLogger.e(TAG, \"timeout\")        ← different message\n"
        "  OUTPUT:\n"
        "    AppLogger.e(TAG, \"connect failed\") ← KEEP (message A)\n"
        "    AppLogger.e(TAG, \"timeout\")        ← KEEP (message B, different!)\n\n"

        "═══════════════════════════════════════════════════════════\n"
        "CONCRETE EXAMPLES (follow these exactly)\n"
        "═══════════════════════════════════════════════════════════\n\n"

        "EXAMPLE 1 — Different levels, same message:\n"
        "  INPUT:\n"
        "    AppLogger.d(MODULE, \"onLoneWorkerServiceStarted\")\n"
        "    Log.e(MODULE, \"onLoneWorkerServiceStarted\")\n"
        "    Log.i(MODULE, \"onLoneWorkerServiceStarted\")\n"
        "  \n"
        "  STEP 1 (convert):\n"
        "    AppLogger.d(MODULE, \"onLoneWorkerServiceStarted\")\n"
        "    AppLogger.e(MODULE, \"onLoneWorkerServiceStarted\")  ← Log.e converted\n"
        "    AppLogger.i(MODULE, \"onLoneWorkerServiceStarted\")  ← Log.i converted\n"
        "  \n"
        "  STEP 2 (deduplicate):\n"
        "    Level d ≠ Level e ≠ Level i → DIFFERENT LEVELS → KEEP ALL 3\n"
        "  \n"
        "  FINAL OUTPUT (3 lines, NOT 1!):\n"
        "    AppLogger.d(MODULE, \"onLoneWorkerServiceStarted\")\n"
        "    AppLogger.e(MODULE, \"onLoneWorkerServiceStarted\")\n"
        "    AppLogger.i(MODULE, \"onLoneWorkerServiceStarted\")\n\n"

        "EXAMPLE 2 — Same level, same message (duplicates):\n"
        "  INPUT:\n"
        "    AppLogger.i(MODULE, \"deviceHasLocationSettingEnabled $v\")\n"
        "    AppLogger.i(MODULE, \"deviceHasLocationSettingEnabled $v\")\n"
        "    AppLogger.i(MODULE, \"deviceHasLocationSettingEnabled $v\")\n"
        "    AppLogger.i(MODULE, \"deviceHasLocationSettingEnabled $v\")\n"
        "    AppLogger.i(MODULE, \"deviceHasLocationSettingEnabled $v\")\n"
        "  \n"
        "  STEP 2 (deduplicate):\n"
        "    All 5 have SAME level (i) and SAME args → DUPLICATES → KEEP 1\n"
        "  \n"
        "  FINAL OUTPUT (1 line):\n"
        "    AppLogger.i(MODULE, \"deviceHasLocationSettingEnabled $v\")\n\n"

        "EXAMPLE 3 — Mixed levels with some duplicates:\n"
        "  INPUT:\n"
        "    AppLogger.i(MODULE, \"deviceHasLocationSettingEnabled $v\")\n"
        "    AppLogger.i(MODULE, \"deviceHasLocationSettingEnabled $v\")\n"
        "    AppLogger.i(MODULE, \"deviceHasLocationSettingEnabled $v\")\n"
        "    AppLogger.i(MODULE, \"deviceHasLocationSettingEnabled $v\")\n"
        "    AppLogger.i(MODULE, \"deviceHasLocationSettingEnabled $v\")\n"
        "    Log.d(MODULE, \"deviceHasLocationSettingEnabled $v\")\n"
        "    Log.e(MODULE, \"deviceHasLocationSettinffdfdffffdfgEnabled $v\")\n"
        "  \n"
        "  STEP 1 (convert):\n"
        "    AppLogger.i(MODULE, \"deviceHasLocationSettingEnabled $v\")  ← 5x same\n"
        "    AppLogger.d(MODULE, \"deviceHasLocationSettingEnabled $v\")  ← Log.d converted\n"
        "    AppLogger.e(MODULE, \"deviceHasLocationSettinffdfdffffdfgEnabled $v\")  ← Log.e converted, typo\n"
        "  \n"
        "  STEP 2 (deduplicate):\n"
        "    - 5x AppLogger.i with same message → MERGE to 1 (same level + same args)\n"
        "    - AppLogger.d has DIFFERENT level (d ≠ i) → KEEP\n"
        "    - AppLogger.e has typo in message (different string) → KEEP\n"
        "  \n"
        "  FINAL OUTPUT (3 lines):\n"
        "    AppLogger.i(MODULE, \"deviceHasLocationSettingEnabled $v\")\n"
        "    AppLogger.d(MODULE, \"deviceHasLocationSettingEnabled $v\")\n"
        "    AppLogger.e(MODULE, \"deviceHasLocationSettinffdfdffffdfgEnabled $v\")\n\n"
        ...
        "  REMOVE: import Logr\n"
        "  REMOVE: import com.st.model.log.STLevelLog\n"
        "  REMOVE: import com.honeywell.domain.managers.loggerApp.AppSTLogger\n"
        "  Keep all other imports unchanged.\n\n"

        "Return ONLY raw source code. NO markdown, NO explanations, NO comments added."
    )
